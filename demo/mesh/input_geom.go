...
func (g *InputGeom) loadMesh(path string) bool {
	...
	common.RcCalcBounds(g.m_mesh.getVerts(), g.m_mesh.getVertCount(), g.m_meshBMin, g.m_meshBMax)
...
func (g *InputGeom) drawConvexVolumes(dd debug_utils.DuDebugDraw, hilights ...bool) {
	...
	for i := 0; i < g.m_volumeCount; i++ {
		...
		k := vol.nverts - 1
		for j < vol.nverts {
			va := common.GetVert3(vol.verts, k)
			vb := common.GetVert3(vol.verts, j)
	...
	for i := 0; i < ncid; i++ {
		...
		for j := 0; j < ntris*3; j += 3 {
			...
			if intersectSegmentTriangle(src, dst,
				...
				common.GetVert3(verts, tris[j+2]), t) {
				if t < *tmin {
					*tmin = t
				}
				hit = true

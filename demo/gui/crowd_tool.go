...
func newCrowdToolState() *CrowdToolState {
	d := &CrowdToolState{
...
rad := c.m_sample.getAgentRadius();
 ...
 nav := c.m_sample.getNavMesh();
 crowd := c.m_sample.getCrowd();
if (nav==nil || crowd==nil){return;}


if (c.m_toolParams.m_showNodes && crowd.GetPathQueue()) {
...
if (c.m_toolParams.m_showDetailAll == false && i != c.m_agentDebug.Idx){continue;}
...
for  j := 0; j < npath; j++{debug_utils.DuDebugDrawNavMeshPoly(&dd, *nav, path[j], debug_utils.DuRGBA(255,255,255,24));}

}
}

if (c.m_targetRef)
	debug_utils.DuDebugDrawCross(&dd, c.m_targetPos[0],c.m_targetPos[1]+0.1,c.m_targetPos[2], rad, debug_utils.DuRGBA(255,255,255,192), 2.0f);
...
if (ag.Ncorners && ag.CornerFlags[ag.Ncorners-1] & recast.DT_STRAIGHTPATH_OFFMESH_CONNECTION) {
...
center := ag.Boundary.GetCenter();
	debug_utils.DuDebugDrawCross(dd, center[0],center[1]+radius,center[2], 0.2, debug_utils.DuRGBA(192,0,128,255), 2.0f);
...
if (common.TriArea2D(pos, s, s+3) < 0.0){
...
if (ag.TargetState == recast.DT_CROWDAGENT_TARGET_REQUESTING || ag.TargetState == recast.DT_CROWDAGENT_TARGET_WAITING_FOR_QUEUE){
	col = debug_utils.DuLerpCol(col, debug_utils.DuRGBA(128,0,255,128), 32);} else if (ag.TargetState == recast.DT_CROWDAGENT_TARGET_WAITING_FOR_PATH){
	col = debug_utils.DuLerpCol(col, debug_utils.DuRGBA(128,0,255,128), 128);} else if (ag.TargetState == recast.DT_CROWDAGENT_TARGET_FAILED){
	col = debug_utils.DuRGBA(255,32,16,128);} else if (ag.TargetState ==recast.DT_CROWDAGENT_TARGET_VELOCITY){
...
if (ag.TargetState == recast.DT_CROWDAGENT_TARGET_REQUESTING || ag.TargetState == recast.DT_CROWDAGENT_TARGET_WAITING_FOR_QUEUE){
	...
	col = debug_utils.DuLerpCol(col,debug_utils.DuRGBA(128,0,255,192), 128);} else if (ag.TargetState == recast.DT_CROWDAGENT_TARGET_FAILED){
...
func (c *CrowdToolState) setMoveTarget(p []float64, adjust bool) {
	...
	if adjust {
		...
		navquery.FindNearestPoly(p, halfExtents, filter, c.m_targetRef, c.m_targetPos)

...
func configureDefaults(windowWidth int, windowHeight int) uint32 {
	// Configure the default glfont vertex and fragment shaders
...
// LoadFontBytes loads the specified glfont bytes at the given scale.
...
// LoadFont loads the specified glfont at the given scale.
...
func (f *Font) Printf(x, y float32, scale float32, fs string, argv ...interface{}) error {
	...
	for i := range indices {
		...
		// skip runes that are not in glfont chacter range
		if !ok {
			fmt.Printf("%c %d\n", runeIndex, runeIndex)
			continue
		}
